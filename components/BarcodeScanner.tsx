      ...
      <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
        ...
        {hasPermission === false && (
          <div className="text-center py-8">
            <div className="text-red-600 mb-4">カメラへのアクセスができません</div>
            {isHttpsRequired && (
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4">
                <div className="text-yellow-800 text-sm">
                  <strong>HTTPS接続が必要です</strong>
                  <p className="mt-2">モバイルデバイスでカメラを使用するには、HTTPS接続が必要です。</p>
                </div>
              </div>
            )}
            <p className="text-sm text-gray-600 mb-4">{errorMessage || 'ブラウザの設定でカメラの使用を許可してください。'}</p>
            ...
            <button onClick={initializeScanner} className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">再試行</button>
          </div>
        )}
        ...
        {hasPermission === true && (
          <>
            <div className="relative mb-4">
              <video ref={videoRef} className="w-full h-64 bg-black rounded" autoPlay playsInline muted />
              {isScanning && (
            ...
            <div className="text-center">
              <p className="text-sm text-gray-600 mb-2">JANコード13桁のバーコードを赤い枠内に合わせてください</p>
              <p className="text-xs text-gray-500">自動的に読み取りが開始されます</p>
            </div>
          </>
        )}

        <div className="mt-4 flex justify-center">
          <button onClick={onClose} className="bg-gray-500 text-white px-6 py-2 rounded hover:bg-gray-600">キャンセル</button>
        </div>
      </div>
    </div>
  );
